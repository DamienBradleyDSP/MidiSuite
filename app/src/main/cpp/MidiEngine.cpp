...
int MidiEngine::generateMidi(uint8_t *midiByteArray,
    ...
    if ((int) midiMessages.size() * 3 > (maxBytesPerBuffer))
        return 0; // If too many messages return
...
void MidiEngine::generateClockSignal(uint8_t *midiByteArray,
                                     MidiController::playPositionInformation &currentPosition) {
...
void MidiEngine::generateMtcQuarterFrame(uint8_t *,
                                         MidiController::playPositionInformation &currentPosition) {
...
// Sequencer / Global Parameters
    ...
    for (int i = 0; i <
                    ProjectSettings::maxNumberOfRhythmModules; i++) // +1 for parameter move operations on gui side
    {
        layout.add(std::string("numberOfBeats" + std::to_string(i)),
                   ProjectSettings::startingNumberOfBeats);
        layout.add(std::string("numberOfBars" + std::to_string(i)),
                   ProjectSettings::startingNumberOfBars);
        layout.add(std::string("selectionOfBeats" + std::to_string(i)),
                   ProjectSettings::startingNumberOfBeats);
