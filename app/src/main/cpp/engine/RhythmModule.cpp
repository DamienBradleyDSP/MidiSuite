...
RhythmModule::RhythmModule(std::map<std::string, std::atomic<float> *> &parameters,
                           int rhythmNumber)
...
void RhythmModule::generateMidi(std::unique_ptr<MidiController> &midiController) {
    ...
    auto beatNum = 1;
    for (auto &&beat: beats) {
        beat->setBarPositionLength((beatNum - 1) * numberOfBars->load() / numberOfBeats->load(),
                                   numberOfBars->load() / numberOfBeats->load());
