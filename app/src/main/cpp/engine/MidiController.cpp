...
void MidiController::calculateBufferSamples(playPositionInformation &currentpositionstruct,
                                            bars totalNumOfBars) {
    ...
    totalSampleLength = (totalNumberOfBars / barsPerMinute) * 60.0 *
                        sampleRate; //length in samples of the total number of bars spanned by the given rhythm modules
    ...
    if (resetLoop->load()) {
        ...
        double ppqOverSpill = (double) currentpositionstruct.ppqPosition -
                              (double) currentpositionstruct.ppqPositionOfLastBarStart +
                              ppqPerBuffer;
        double ppqFromReset = 4.0 - ((double) currentpositionstruct.ppqPosition -
                                     (double) currentpositionstruct.ppqPositionOfLastBarStart);
...
void MidiController::addMidiMessage(dbMidiMessage &noteOnMessage, bars noteOnPosition,
                                    dbMidiMessage &noteOffMessage, bars noteOffPosition,
                                    bool sustain) {
    ...
    if (sustainToNextNote) {
        for (auto &&message: sustainedMidiMessages) {
            ...
            if (message.getNoteNumber() != noteOnMessage.getNoteNumber())
                bufferMidiMessages.push_back(message);
        }
...
int MidiController::getLocation(bars barPosition) {
    ...
    if (sampleLocation >= sampleSpanOverspill.first &&
        sampleLocation < sampleSpanOverspill.second) {
