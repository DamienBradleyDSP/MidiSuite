...
public:
    ...
    void
    calculateBufferSamples(playPositionInformation &currentpositionstruct, bars totalNumberOfBars);

    void applyMidiMessages(std::list<dbMidiMessage> &buffer);

    // Beat Interface
    void
    addMidiMessage(dbMidiMessage &noteOnMessage, bars noteOnPosition, dbMidiMessage &noteOffMessage,
                   bars noteOffPosition, bool sustain = false);
