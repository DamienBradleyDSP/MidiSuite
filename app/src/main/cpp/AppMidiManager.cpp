...
void updatePlayPosition() {
    ...
    currentPosition.ppqPosition = quarternotespersample *
                                  frameCounter;    // calculates current number of quarter notes - this will be done by the struct
    currentPosition.ppqPositionOfLastBarStart = floor(frameCounter * barspersample) *
                                                4;    // calculates number of quarter notes at start of last bar, 4/4 time
...
void midiEngineCallback(SLAndroidSimpleBufferQueueItf bq, void *context) {
    ...
    assert(maximumMidiBytesPerBuffer <
           maxAmidiBufferSize); // Midi bytes per buffer should be less than AMIDI's max, in all reasonable cases
    auto numberOfBytesSent = midiEngine.generateMidi(byteArray, currentPosition);

    AMidiInputPort_sendWithTimestamp(sMidiInputPort, byteArray, numberOfBytesSent,
                                     timeOfMidiStart); // send without timestamp just calls send with timestamp
...
Java_com_example_nativemidi_AppMidiManager_createBufferQueueAudioPlayer(JNIEnv, jclass,
    ...
    result = (*bqPlayerBufferQueue)->RegisterCallback(bqPlayerBufferQueue, midiEngineCallback,
                                                      NULL);
    assert(SL_RESULT_SUCCESS == result);
    (void) result;
    ...
    maximumMidiBitsPerBuffer =
            (double) midiBaudRate / ((double) sampleRate / (double) bqPlayerBufSize);
